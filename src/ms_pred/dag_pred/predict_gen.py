...
def get_args():
    ...
    parser.add_argument(
        "--pubchem-map-path", default="data/pubchem/pubchem_formulae_inchikey.hdf5"
    )
...
def predict():
    ...
    if kwargs["num_decoys"] == 0:
        ...
        all_save_path = [
            save_dir / f"decoy_tree_preds_chunk_{i}.hdf5"
            for i in range(kwargs["num_decoys"])
        ]
    ...
    with torch.no_grad():
        ...
        def prepare_entry(entry):
            ...
            collision_energies = [
                i for i in ast.literal_eval(entry["collision_energies"])
            ]
            ...
            if num_decoys > 0:  # decoys only
                ...
                # mutate molecules
                decoy_mols = [
                    mutate(mol, mutation_rate=1.0) for _ in range(num_mutation)
                ]
                ...
                if num_pubchem > 0:
                    ...
                    decoy_mols += [
                        Chem.MolFromSmiles(_[0])
                        for _ in random.choices(smi_inchi_list, k=num_pubchem)
                    ]
                ...
                decoy_smis = []
                for new_mol, new_inchikey in zip(
                    decoy_mols[unique_inds], decoy_inchikeys[unique_inds]
                ):
                ...
                for i, decoy_smi in enumerate(decoy_smis):
                    for colli_eng in collision_energies:
                        colli_eng_val = common.collision_energy_to_float(
                            colli_eng
                        )  # str to float
                        ...
                        tup_to_process.append(
                            (
                                ...
                                precursor_mz,
                                out_h5_key,
                                int(common.str_to_hash(name), base=16)
                                % len(all_save_path),  # output hdf5 index
                            )
                        )
        ...
        random.shuffle(
            predict_entries
        )  # shuffle to evenly distribute graph size across batches
        logging.info(f"There are {len(predict_entries)} entries to process")

        batch_size = kwargs["batch_size"]
        all_batched_entries = [
            predict_entries[i : i + batch_size]
            for i in range(0, len(predict_entries), batch_size)
        ]
        ...
        def producer_func(batch):
            ...
            if gpu and avail_gpu_num >= 0:
                if kwargs["num_workers"] > 0:
                    worker_id = multiprocess.process.current_process()._identity[
                        0
                    ]  # get worker id
            ...
            smi, name, colli_eng_val, adduct, precursor_mz, out_name, out_file_idx = (
                list(zip(*batch))
            )
            ...
            for (
                ...
                _out_name,
                _out_file_idx,
            ) in zip(smi, name, pred, colli_eng_val, adduct, out_name, out_file_idx):
                output = {
                ...
                return_list.append(
                    (_out_name, json.dumps(output, indent=2), _out_file_idx)
                )
            return return_list

        def write_h5_func(out_entries):
            h5s = [
                common.HDF5Dataset(save_path, mode="w") for save_path in all_save_path
            ]
        ...
        if kwargs["num_workers"] == 0:
            output_entries = [
                producer_func(batch) for batch in tqdm(all_batched_entries)
            ]
