...
class FragGNN(pl.LightningModule):
        ...
        **kwargs,
        ...
        if self.embed_adduct:
            ...
            if self.embed_elem_group:
                adduct_modes = len(
                    set([j for i in common.ion_pos2extra_multihot.values() for j in i])
                )
        ...
        if self.embed_collision:
            ...
            self.collision_embedder_denominators = nn.Parameter(
                torch.pow(pe_scalar, pe_power)
            )
            ...
            with root_repr.local_scope():
                ...
                if self.embed_collision:
                    embed_collision = torch.cat(
                        (
                            torch.sin(
                                collision_engs.unsqueeze(1)
                                / self.collision_embedder_denominators.unsqueeze(0)
                            ),
                            torch.cos(
                                collision_engs.unsqueeze(1)
                                / self.collision_embedder_denominators.unsqueeze(0)
                    ...
                    embed_collision = (
                        torch.where(  # handle entries without collision energy (== nan)
    ...
    def predict_mol(
        ...
        engine = [
            fragmentation.FragmentEngine(rsmi, mol_str_canonicalized=canonical_root_smi)
            for rsmi in root_smi
        ]
        ...
        root_form_vec = torch.FloatTensor(
            np.array([common.formula_to_dense(rf) for rf in root_form])
        ).to(device)
        adducts = torch.LongTensor(
            [common.ion2onehot_pos[a] if type(a) is str else a for a in adduct]
        ).to(device)
        ...
        root_graph_dict = [
            ...
            )  # add random walk feature later in batched
        ...
        if self.root_encode == "gnn":
            ...
            root_fp = torch.from_numpy(
                np.array([common.get_morgan_fp_smi(rsmi) for rsmi in root_smi])
            )
        ...
        with torch.no_grad():
            ...
            while depth < max_depth:
                ...
                for rev_i, (st, e, rg) in enumerate(
                    ...
                    for i in st:
                        ...
                        if len(info["new_to_old"]) > 1:
                            ...
                            batched_old_edge_idx.append(
                                batched_old_edge_idx[-1] + rg["graph"].number_of_edges()
                            )
                if len(new_info_dicts) == 0:
                    break
                stack = new_stack
                batched_select = torch.from_numpy(np.concatenate(batched_select)).to(
                    device
                )
                batched_num_nodes = torch.LongTensor(batched_num_nodes).to(device)
                batched_old_edge_idx = torch.LongTensor(batched_old_edge_idx[1:]).to(
                    device
                )
                ...
                batched_num_edges = torch.bincount(
                    torch.bucketize(
                        frag_batch.edata[dgl.EID], batched_old_edge_idx, right=True
                    )
                )
                ...
                new_frag_hashes = [
                    engine[ri].wl_hash(i) for i, ri in zip(stack, reverse_idx)
                ]
                ...
                self.tree_processor.add_pe_embed(
                    frag_batch
                )  # add random walk feature. GPU memory intensive!
                inds = torch.tensor(reverse_idx).long().to(device)

                broken_nums_ar = np.array(
                    [
                        frag_hash_to_entry[ri][i]["max_broken"]
                        for i, ri in zip(new_frag_hashes, reverse_idx)
                    ]
                )
                ...
                if (
                    depth == max_depth and not decode_final_step
                ):  # return an unprocessed list
                    ...
                    for rev_idx in range(batch_size):
                        frag_hash_to_entry[rev_idx] = new_vals[rev_idx][
                            "frag_hash_to_entry"
                        ]
                        frag_to_hash[rev_idx] = new_vals[rev_idx]["frag_to_hash"]
                        form_to_min_score[rev_idx] = new_vals[rev_idx][
                            "form_to_min_score"
                        ]
...
def auto_regressive_decode(
    ...
    for new_item in sorted_order:
        ...
        for out_dict in out_dicts:
            ...
            cur_probs = sorted([i["prob_gen"] for i in frag_hash_to_entry.values()])[
                ::-1
            ]
